...
This application provides the multi-tenant FHIR back end for the [OCTRI FHIR Sandbox application][fhir-sandbox-repo]. See the following repository for the application that manages sandboxes, SMART on FHIR clients, and OAuth 2.0 authorization.

https://github.com/OHSU-OCTRI/fhir-sandbox

## Configuration

Copy `env.sample` to `.env` and update as desired. The default `.env` values set the PostgreSQL database credentials to match `docker-compose.yml`. Because the application is commonly developed along with the [OCTRI FHIR Sandbox application][fhir-sandbox-repo], the server port defaults to 8000 to avoid a conflict with that application. This can be adjusted if that does not work for your environment.
...
[fhir-sandbox-repo]: https://github.com/OHSU-OCTRI/fhir-sandbox
